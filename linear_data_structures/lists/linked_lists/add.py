...
# object. Line 6 creates a new node and places the item as its data. Now we must complete the
# process by linking the new node into the existing structure. This requires two steps. Step 1
# (line 7) changes the next reference of the new node to refer to the old first node of the list.
# Now that the rest of the list has been properly attached to the new node, we can modify the head
# of the list to refer to the new node. Step 2 (line 8) sets the head of the list.
